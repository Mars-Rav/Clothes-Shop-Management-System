...
public abstract class CashierController {
	...
	static File test = new File("C:\\Users\\Tech Line\\Desktop\\boutique v.2.0\\boutique\\src\\Controllers\\staff.txt");
...
//        staff.stream().forEach(p-> System.out.println(p));
    ...
    public static void stafftable( ArrayList<User> mystaff){
        System.out.printf("+------------------------------------------------------------------------------------------------------------------------------------------------------------+%n");
        ...
        System.out.println("      +");
        System.out.printf("+------------------------------------------------------------------------------------------------------------------------------------------------------------+%n");
        for( User user: mystaff){
            System.out.print("+");
            System.out.format("%5s %10s %15s %15s %15s %20s %15s %15s %15s %15s ",
                    user.getID(), user.getName(), user.getGender(), user.getDob(),
                    user.getPhoneNumber(),user.getEmail(),user.getSalary(),user.getAddress(),
   ...
   public static ArrayList<GarmentModel> garments = GarmentController.getGarments();
    ...
    public static void addGarmentToInvoice(User cashier) throws IOException, ParseException {
        ...
        while(true) {
        	System.out.println("enter the name of garment /for cansle inter Exit");
...
//    }
    ...
    public static void removeCashier(String name) throws IOException, ParseException {
        readFromFile();
        staff.removeIf(user -> name.equals(user.getName()));
        add();
